@extends("layouts.admin.layout")

@section('admin-content')


    <div class="content-wrapper">

        <section class="content-header">

            {{-- <h1>@lang('site.users')</h1> --}}
            <h1>Users</h1>
            <ol class="breadcrumb">
                {{-- <li><a href="{{ route('dashboard.welcome') }}"><i class="fa fa-dashboard"></i> @lang('site.dashboard')</a></li>
                <li class="active">@lang('site.users')</li> --}}
                <li><a href="#"><i class="fa fa-dashboard"></i> dashboard</a></li>
                <li class="active">dashboard users</li>
            </ol>
        </section>

        <section class="content">

            <div class="box box-primary">

                <div class="box-header with-border">

                    {{-- <h3 class="box-title" style="margin-bottom: 15px">@lang('site.users') <small>{{ $users->total() }}</small></h3> --}}
                    <h3 class="box-title" style="margin-bottom: 15px">users<small>total users</small></h3>

                    <form action="" method="get">

                        <div class="row">

                            <div class="col-md-4">
                                {{-- <input type="text" name="search" class="form-control" placeholder="@lang('site.search')" value="{{ request()->search }}"> --}}
                                <input type="text" name="search" class="form-control" placeholder="lang search" value="{{ request()->search }}">
                            </div>

                            <div class="col-md-4">
                                <button type="submit" class="btn btn-primary"><i class="fa fa-search"></i> lang searc</button>
                                {{-- @if (auth()->user()->hasPermission('create_users'))
                                    <a href="" class="btn btn-primary"><i class="fa fa-plus"></i> lang add</a>
                                @else --}}
                                    <a href="" class="btn btn-primary disabled"><i class="fa fa-plus"></i>  lang add </a>
                                {{-- @endif --}}
                            </div>

                        </div>
                    </form><!-- end of form -->

                </div><!-- end of box header -->

                <div class="box-body">

                    @if ($user->count() > 0)

                        <table class="table table-hover">

                            <thead>
                            <tr>
                                <th>#</th>
                                <th> lang name</th>
                                <th>lang email</th>
                                <th>lang action</th>
                            </tr>
                            </thead>
                            
                            <tbody>
                            @foreach ($user as $index=>$user)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td>{{ $user->name }}</td>
                                    <td>{{ $user->email }}</td>
                                    {{-- <td><img src="{{ $user->image_path }}" style="width: 100px;" class="img-thumbnail" alt=""></td> --}}
                                    <td>
                                        {{-- @if (auth()->user()->hasPermission('update_user'))
                                            <a href="" class="btn btn-info btn-sm"><i class="fa fa-edit"></i> lang edit</a>
                                        @else --}}
                                            <a href="" class="btn btn-info btn-sm"><i class="fa fa-edit"></i> lang edit </a>
                                        {{-- @endif
                                        @if (auth()->user()->hasPermission('delete_users')) --}}
                                            <form action="" method="post" style="display: inline-block">
                                                {{ csrf_field() }}
                                                {{ method_field('delete') }}
                                                <button type="submit" class="btn btn-danger delete btn-sm"><i class="fa fa-trash"></i> lang delete</button>
                                            </form><!-- end of form -->
                                        {{-- @else
                                            <button class="btn btn-danger btn-sm disabled"><i class="fa fa-trash"></i> lang edit delete</button>
                                        @endif --}}
                                    </td>
                                </tr>
                            
                            @endforeach
                            </tbody>

                        </table><!-- end of table -->
                        
                        {{-- {{ $user->appends(request()->query())->links() }} --}}
                        
                    @else
                        
                        <h2>@lang('site.no_data_found')</h2>
                        
                    @endif

                </div><!-- end of box body -->


            </div><!-- end of box -->

        </section><!-- end of content -->

    </div><!-- end of content wrapper -->


@endsection
